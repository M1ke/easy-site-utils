...
function make_html($text,$tags=null,$multi_byte=false){
	$text=string_check($text,false,true,$multi_byte);
	if (!empty($tags)){
		$text=strip_tags($text,$tags);
	}
	$text=str_replace(array('// <![CDATA['."\r\n","\r\n".'// ]]>'),'',$text);
	...
	// if the line breaks are odd it can start a paragraph before a close tag, or close one afterwards, which isn't good
	$text=preg_replace('!<p>\s*(</?(?:'.$htmltags.')[^>]*>)!',"$1",$text);
	$text=preg_replace('!(</?(?:'.$htmltags.')[^>]*>)\s*</p>!',"$1",$text);
	// turn any remaining single breaks into <br/> tags, then remove them from after block elements (or before the close tag)
