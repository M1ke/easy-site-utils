...
function file_name_uploaded($file, $return = null){
	...
	$name = substr($file, 0, $dot);
	$name = str_replace(' ', '-', $name);
	$file = ['ext' => $ext, 'name' => $name, 'temp' => $temp];
...
/**
 * Replace all new line characters in given string with passed value
 *
 * @param string $string
 ...
 * @return string
 */
function line_breaks_replace($string, $replace_value){
...
function make_email(&$string, $blank = false){
	...
	if (strlen($string)>0){
		...
		$pattern = "/\b['a-z0-9_%+-]+(?:\.['a-z0-9_%+-]+)*@[a-z0-9.-]+\.[a-z]{2,}\b(?!\S)/";
...
function permalink_from_caps($string){
	$chars = [];
...
function permalink_to_caps($string, $camel_case = false){
	$studly = explode('-', $string);
	for ($n = $camel_case ? 1 : 0; $n<count($studly); $n++){
		$studly[$n] = ucfirst($studly[$n]);
	}
	$studly = implode('', $studly);
...
function select_options($array, $options = ['string' => ' selected']){
	...
	foreach ($array as $item){
		...
		if ($options['extra']){
			$select .= str_replace('%val%', $values[1], $options['extra']);
		}
...
function shorten($string, $length = 20, $nospan = null){
	if (strlen($string)>$length+1){
		...
		if (!$nospan){
			$short = '<span title="'.$string.'">'.$short.'</span>';
		}
	}
	else {
		$short = $string;
